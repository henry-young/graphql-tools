            ...
            if (selection.kind === graphql_1.Kind.FIELD) {
                ...
                field.args.forEach(function (argument) {
                    if (argument.name in args) {
                        ...
                        newVariables[variableName] = serializeArgumentValue(argument.type, args[argument.name]);
                    }
                });
...
function serializeArgumentValue(type, value) {
    if (value == null) {
        return null;
    }
    var nullableType = graphql_1.getNullableType(type);
    if (nullableType instanceof graphql_1.GraphQLEnumType) {
        return nullableType.serialize(value);
    }
    else if (nullableType instanceof graphql_1.GraphQLScalarType) {
        return value;
    }
    else if (nullableType instanceof graphql_1.GraphQLList) {
        return value.map(function (v) { return serializeArgumentValue(nullableType.ofType, v); });
    }
    else if (nullableType instanceof graphql_1.GraphQLInputObjectType) {
        var fields_1 = nullableType.getFields();
        return Object.keys(value).reduce(function (acc, k) {
            var v = value[k];
            acc[k] = serializeArgumentValue(fields_1[k].type, v);
            return acc;
        }, {});
    }
}
